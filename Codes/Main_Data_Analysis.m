...
axis([0.5,2.5,0,500])
xlabel('Time (s)');
ylabel('Absolute linear acceleration (cm/s^2)');
...
set(gcf, 'Position', [680   530   327   348]);
xlabel('Time (s)');
ylabel('Absolute Angular Speed (rad/s)');
...
ylim(yl); hold off;
ylabel('Frequency of interaction (Hz)');
...
ylim(yl); hold off;
ylabel('Duration of interaction (s)');
...
ylim(yl); hold off;
ylabel('Total time of interaction (s)');
...
xlabel('Delay steps', 'Interpreter', 'latex');
ylabel('max TE', 'Interpreter', 'latex');
...
% Formatting
xlabel('Transfer Entropy (TE)', 'Interpreter', 'latex', 'FontSize', 22);
ylabel('Probability Density', 'Interpreter', 'latex', 'FontSize', 22);
